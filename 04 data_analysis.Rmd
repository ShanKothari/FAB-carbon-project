...
subtitle: "Appendix S1: Statistical analyses"
author:
  - Reb L. Bryant, Shan Kothari, Jeannine Cavender-Bares, Stephanie J. Curran,
...
The GitHub repository housing this document can be found at [this link](https://github.com/ShanKothari/FAB-carbon-project). A stable archived version can be found at @kothari2024.
...
The archived data can be found at the EDI Data Portal (@bryant2023). After setting the working directory to where we saved the data file, we can read it in.
...
-   species richness
-   @ll2010's functional dispersion (FDis), as reported by @grossman2017
-   @helmus2007's phylogenetic species variability (PSV), as reported by @grossman2017

The traits that went into the calculation of FDis include: leaf mass per area, leaf nitrogen concentration, leaf habit (categorical), wood density, mycorrhizal type (categorical), leaf calcium concentration, and indices of shade, drought, and waterlogging tolerance from @nv2006. Full details are found in the paper by @grossman2017.
...
Although many PLFAs were measured (and @grossman2019 has further details), we selected certain PLFAs for examination in the analyses below.
...
As explained in the main text and Appendix S2, we proposed an initial model based on *a priori* hypotheses and earlier data analyses. The structure of this model is as follows:
...
As the Fisher's C *p*-value \> 0.05 shows, this model fits better than the one above. The in-depth interpretation of this model is in the main text. We can check each of the submodels of the full SEM to make sure that it means standard assumptions.
