...
As it turns out, soil pH is positively correlated both with soil C accumulation and (more weakly) with the soil C pool in 2019.

```{r soilC_pH, warning = F, message = F}
## block random intercept yields singular fit
summary(lm(pH~soilC,data=carbon_seq))
ggplot(data=carbon_seq,aes(x=soilC,y=pH,color=as.factor(block)))+
  geom_point(size=2)+geom_smooth(method="lm",se=F,linewidth=2)

## block random intercept yields singular fit
summary(lm(pH~soilC_2019,data=carbon_seq))
ggplot(data=carbon_seq,aes(x=soilC_2019,y=pH,color=as.factor(block)))+
  geom_point(size=2)+geom_smooth(method="lm",se=F,linewidth=2)
...
check_model(lm(pH~soilC,data=carbon_seq))
check_model(lm(pH~soilC_2019,data=carbon_seq))
```
...
Lastly we might wonder whether macroaggregates (or soil moisture, or pH) are correlated with fine root C. Once again, the answer is no: we show this for macroaggregates below.
...
## Tree biodiversity effects on above- and belowground C accumulation
...
First we read in the data from Grossman et al. (2019), available at DOI [https://doi.org/10.6073/pasta/3918fe1d3aab00ab73c4a83372cb82cf](10.6073/pasta/3918fe1d3aab00ab73c4a83372cb82cf) or [https://portal.edirepository.org/nis/mapbrowse?scope=knb-lter-cdr&identifier=683](this link).

```{r readPLFAs}
PLFA<-read.csv("FAB_AMF_Plots_V3.csv")

```

For convenience, we will attach the PLFA data to the main data frame.

```{r attachPLFAs}
PLFA_match<-match(carbon_seq$plot,PLFA$Community)
carbon_seq$X17.0_anteiso.pp<-PLFA$X17.0_anteiso.pp[PLFA_match]
carbon_seq$X17.0_anteiso.pm<-PLFA$X17.0_anteiso.pm[PLFA_match]
carbon_seq$X18.0_10me.pp<-PLFA$X18.0_10me.pp[PLFA_match]
carbon_seq$X18.0_10me.pm<-PLFA$X18.0_10me.pm[PLFA_match] ## this one works
carbon_seq$X18.1_w9t.pp<-PLFA$X18.1_w9t.pp[PLFA_match]
carbon_seq$X18.1_w9t.pm<-PLFA$X18.1_w9t.pm[PLFA_match]
carbon_seq$X18.2_w6.9c.pp<-PLFA$X18.2_w6.9c.pp[PLFA_match]
carbon_seq$X18.2_w6.9c.pp<-PLFA$X18.2_w6.9c.pm[PLFA_match]

carbon_seq$gen_fungi_pct<-PLFA$gen_fungi_pct[PLFA_match]
carbon_seq$gram_pos_pct<-PLFA$gram_pos_pct[PLFA_match]
carbon_seq$gram_neg_pct<-PLFA$gram_neg_pct[PLFA_match]
carbon_seq$actinos_pct<-PLFA$actinos_pct[PLFA_match]

carbon_seq$Biomass.pm<-PLFA$Biomass.pm[PLFA_match]
carbon_seq$Biomass.nm<-PLFA$Biomass.nm[PLFA_match]

carbon_seq$fungal.bacterial_mass<-PLFA$fungal.bacterial_mass[PLFA_match]
carbon_seq$gram_neg.gram_post_mass<-PLFA$gram_neg.gram_post_mass[PLFA_match]
```

```{r attachPLFAs_2, include=F}

```

## Structural equation modeling of aboveground and soil C
